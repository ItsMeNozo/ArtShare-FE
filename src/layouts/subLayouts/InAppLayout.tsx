...
const InAppLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className={`relative flex h-screen min-h-0 w-full flex-row`}>
      ...
      >
        <Header />
        <div className="from-mountain-100/80 dark:from-mountain-1000 dark:to-mountain-900 border-mountain-100 min-h-0 flex-1 rounded-t-3xl border bg-gradient-to-b to-white transition-shadow duration-300 dark:border-black">
          {children}
        </div>
      </div>
    </div>
